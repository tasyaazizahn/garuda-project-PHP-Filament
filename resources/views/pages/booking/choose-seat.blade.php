                ...
                <div class="relative flex flex-col justify-end">
                    ...
                    <form action="{{ route('booking.confirmSeat', $flight->flight_number) }}" method="POST"
                        ...
                        <div id="Seats-Options"
                            class="flex flex-wrap w-full gap-y-8 @if ($tier->class_type == 'bussiness') gap-x-10 px-[33px] @else gap-x-[14px] @endif">
                            @foreach ($flight->seats->where('class_type', $tier->class_type) as $seat)
                                <label
                                    class="group relative flex w-[55px] h-[52.25px] shrink-0 @if ($tier->class_type == 'bussiness') [&:nth-child(4n+2)]:mr-10 @else [&:nth-child(6n+3)]:mr-[46px] @endif"
